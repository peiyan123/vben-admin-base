...
class timeSlider extends CommonUtils {
  ...
  initRootDomEvent() {
    ...
    this.rootDom.addEventListener('mousemove', (e) => {
      if (that.isMouseDown && this.precision !== 3600) {
        ...
        // 滚动到临界值
        let isRightOver =
          this.allAxisLength + that.containerDom.offsetLeft < 20 + this.rootDom.offsetWidth;
    ...
    this.rootDom.addEventListener('mouseup', (e) => {
      ...
      this.hander(this.presentSeconds);
    ...
    this.rootDom.addEventListener('mousewheel', (e) => {
      ...
      if (this.allAxisLength + this.rootDom.offsetLeft <= document.body.clientWidth) {
        this.rootDom.style.left =
          -this.allAxisLength + document.body.clientWidth - 2 * this.paddingLeft + 'px';
      }
  ...
  handleClick(left) {
    this.presentSeconds =
      ((-this.domLeftToNumberUtils(this.containerDom) + left - this.PADDINGLEFT) *
        this.DAYSECONDS) /
      this.allAxisLength;
 ...
 */
  calTimeSlider() {
    ...
    this.allAxisLength =
      axisDom.offsetWidth * this.precisionSetting[this.wheelIndexMap[this.wheelIndex]];
  ...
  */
  ...
  zoomSetTimeSlider(oldAxisLength, oldTimeLineLeft, newAxisLength) {
    ...
    let left =
      ((oldTimeLineLeft - this.PADDINGLEFT - curRootDomLeft) / oldAxisLength) * newAxisLength -
      (oldTimeLineLeft - this.PADDINGLEFT);
  ...
  timeLinePlay() {
    ...
    this.timer = setInterval(() => {
      ...
      let left =
        this.speed * (this.allAxisLength / this.DAYSECONDS) +
        this.domLeftToNumberUtils(this.timeLineDom);
      this.setTimeLineLeft();
  ...
  getTimeChunkLastEndTime() {
    return this.timeTranslateSecondsUtils(
      this.timeChunkArray[0] && this.timeChunkArray[0].split('-')[1],
    );
  }
  /**
   * 获取时间块的第一段的开始时间
  ...
  getTimeChunkFirstStartTime() {
    return this.timeTranslateSecondsUtils(
      this.timeChunkArray[this.timeChunkArray.length - 1] &&
        this.timeChunkArray[this.timeChunkArray.length - 1].split('-')[0],
    );
  }
  /**
   * 获取时间块的第一段的结束时间
  ...
  getTimeChunkFirstEndTime() {
    return this.timeTranslateSecondsUtils(
      this.timeChunkArray[this.timeChunkArray.length - 1] &&
        this.timeChunkArray[this.timeChunkArray.length - 1].split('-')[1],
    );
  }
}
